classdef Granulator < handle
    ...
    methods
        ...
        function out_frame = process(obj, in_frame, t)
            ...
            in_semitones = floor(t.PitchKnob.Value); %<---
            ...
            for samp = 1:obj.frame_size
                if obj.grain_index > obj.grain_size/2
                    ...
                    new_grain = obj.buffer(grain_start:grain_end);
                    if in_semitones ~= 0
                        new_grain = shiftPitch(new_grain,in_semitones);
                    end
                    if reverse
                        obj.grain_buffer = obj.grain_buffer + hann(obj.grain_size).*flip(new_grain);
                    else
                        obj.grain_buffer = obj.grain_buffer + hann(obj.grain_size).*new_grain;
                    end 
...
%                     pr = roots(pc);
%                     
%                     obj.grain_buffer = filter(zc, pc, obj.grain_buffer*0.5, [],2); % Resonant LPF Filter
